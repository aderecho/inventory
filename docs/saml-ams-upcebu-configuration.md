...
Prefer importing the authoritative AMS metadata XML before entering values manually. In the configuration form, paste the XML into **IdP Metadata XML** and select **Import metadata**. Rooms extracts and stores:

- IdP Entity ID
- HTTP-Redirect SSO URL
- SLO URL, when published
- X.509 signing certificate

Review the extracted fields before saving. The server parses the metadata again during save and treats the stored XML as the source record. Manual configuration remains available when AMS cannot provide metadata XML.
