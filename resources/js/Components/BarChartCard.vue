...
const props = defineProps({
    stats: {
        type: Object,
        default: () => ({ items: 0, receipts: 0, items_per_month: {}, receipts_per_month: {} }),
    },
});
...
const monthlyItems = Array.from({ length: 7 }, (_, i) => props.stats.items_per_month[i + 1] ?? 0);
const monthlyReceipts = Array.from({ length: 7 }, (_, i) => props.stats.receipts_per_month[i + 1] ?? 0);
                ...
                datasets: [
                    {
                        ...
                        data: monthlyItems,
                        backgroundColor: 'rgba(133, 0, 56, 0.1)',
                        borderColor: 'rgb(133, 0, 56)',
                        borderWidth: 2,
                        fill: true,
                        tension: 0.3,
                    },
...
<template>
    <div class="w-full mx-auto flex flex-col lg:flex-row flex-wrap gap-4 sm:gap-5 h-auto">
        <!-- Items Card -->
        <div class="flex-1 min-w-[300px] bg-white p-4 sm:p-5 md:p-6 rounded-lg shadow-lg w-full">
            <div class="flex items-center justify-between mb-2">
                ...
                <span class="text-sm font-semibold text-white bg-[#850038] px-3 py-1 rounded-full">
                    {{ stats.items }} Total
                </span>
            </div>
