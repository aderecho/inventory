...
export function usePermissions() {
    ...
    const disposalActions = computed(() => {
        const result = [];
        if (can("view disposal")) result.push("view");
        return result;
    });
    ...
    const inspectionActions = computed(() => {
        const result = [];
        if (can("view inspections")) result.push("view");
        return result;
    });

    // Api clients
    const apiClientActions = computed(() => {
        const result = [];
        if (can("view api_clients")) result.push("view");
        return result;
    });
    ...
    // Audit logs
    const auditLogActions = computed(() => {
        const result = [];
        if (can("view audit_logs")) result.push("view");
        return result;
    });
    ...
    // Acknowledgements
    const canViewAcknowledgements = computed(() =>
        can("view acknowledgements"),
    );
    const canCreateAcknowledgements = computed(() =>
        can("create acknowledgements"),
    );
    const canShowAcknowledgements = computed(() =>
        can("show acknowledgements"),
    );
    const canUploadAcknowledgements = computed(() =>
        can("upload acknowledgements"),
    );
    ...
    const permissionActions = computed(() => {
        const result = [];
        if (can("view permissions")) result.push("view");
        if (can("edit permissions")) result.push("edit");
        if (can("delete permissions")) result.push("delete");
        return result;
    });

    const canCreatePermission = computed(() => can("create permissions"));
