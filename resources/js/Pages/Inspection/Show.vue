...
<template>
    <Head title="UP | Inspection History" />
    ...
    <Layout>
        ...
        <!-- Item summary card -->
        <div class="mb-5 bg-white rounded-xl border border-gray-200 p-5">
            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div class="flex-1">
                    ...
                    <p v-if="item.description" class="text-sm text-gray-500 mb-3">
                        {{ item.description }}
                    </p>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-x-6 gap-y-2 text-sm">
                        ...
                        <div>
                            ...
                            <p class="text-gray-700 text-xs">
                                {{
                                    item.unit_cost != null
                                        ? `₱${Number(item.unit_cost).toLocaleString("en-PH", {
                ...
                <div class="flex-shrink-0">
                    <p class="text-xs text-gray-400 mb-1">Current condition</p>
                    <div
                        v-if="item.inspections?.[0]?.asset_condition?.condition_name"
                        class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border"
                        :class="
                            conditionBadgeClass(
                                item.inspections[0].asset_condition.condition_name,
                            )
                                .replace('text-', 'border-')
                                .split(' ')[0] +
                            ' ' +
        ...
        <div>
            <h2 class="text-sm font-semibold text-gray-700 mb-4">
                ...
                <span class="ml-1.5 text-xs font-normal text-gray-400">
                    ...
                    {{ (item.inspections?.length ?? 0) === 1 ? "record" : "records" }}
                </span>
            </h2>

            <div v-if="item.inspections && item.inspections.length > 0" class="relative">
                <!-- Vertical line -->
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
                                <div>
                                    <div class="flex items-center gap-2">
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium"
                                            :class="conditionBadgeClass(entry.asset_condition?.condition_name)"
                                        >
                                            {{ entry.asset_condition?.condition_name ?? "Unknown condition" }}
                                        </span>
                                        <span
                                            v-if="index === 0"
            ...
            <!-- Empty state -->
            <div v-else class="bg-white rounded-xl border border-gray-200 px-5 py-16 text-center">
                <i class="fa-solid fa-clipboard-check text-2xl text-gray-300 mb-2"></i>
