...
function submitUpload(personId) {
    ...
    if (form.acknowledgement_item_ids.length === 0) {
        toast.add({ severity: "warn", summary: "No items", detail: "Please select at least one item.", life: 3000 });
        return;
    }
    ...
    form.post(route("acknowledgements.upload-file"), {
        forceFormData: true,
        onSuccess: () => {
            toast.add({ severity: "success", summary: "Uploaded", detail: "Receipt uploaded successfully.", life: 3000 });
            selectedItems.value[personId] = [];
            form.reset();
        },
        onError: (errors) => {
            const first = Object.values(errors)[0];
            toast.add({ severity: "error", summary: "Failed", detail: first, life: 4000 });
        },
    });
}
        ...
        >
            ...
            <div class="flex items-start justify-between mb-6">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">
                        Issued by: {{ receipt.issued_by?.user_profiles?.full_name ?? 'N/A' }}
                    </p>
                </div>
                <button
                    @click="closeWithAnimation"
            ...
            >
                ...
                <div class="flex items-center gap-3 mb-4">
                    <div class="w-9 h-9 rounded-full bg-[#FAECE7] text-[#993C1D] flex items-center justify-center text-sm font-medium">
                        {{ group.person.full_name?.charAt(0) ?? '?' }}
                    </div>
                    <div>
                        <p class="font-medium">{{ group.person.full_name }}</p>
                        <p class="text-xs text-gray-400">{{ group.items.length }} item(s)</p>
                    </div>
                </div>
                ...
                <div class="divide-y mb-4">
                    ...
                    >
                        ...
                        <div class="flex-1">
                            <p class="text-sm font-medium">{{ item.inventory_items?.item_name ?? 'N/A' }}</p>
                            <p class="text-xs text-gray-400">{{ item.inventory_items?.property_number ?? 'N/A' }}</p>
                        </div>

                        <div v-if="item.file" class="flex items-center gap-2">
                            <span class="text-xs text-green-600 font-medium">{{ item.file.file_path.split('/').pop() }}</span>
                            <button
                ...
                <!-- Upload zone -->
                <div
                    v-if="hasSelection(group.person.id)"
                    class="border border-dashed rounded-lg p-4 flex items-center gap-4 bg-gray-50"
                >
                    <div class="flex-1">
                        <p class="text-sm font-medium">
                            Upload receipt for {{ selectedItems[group.person.id]?.length }} selected item(s)
                        </p>
                        <input
                            type="file"
                    ...
                    >
                        {{ getForm(group.person.id).processing ? 'Uploading...' : 'Upload' }}
                    </button>
                </div>
